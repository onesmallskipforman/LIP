...
uint8_t Position_Target;//用于标记位置控制的时间
uint8_t Swing_up=1; //用于标记手动起摆时，是否是第一次进入手动起摆函数
...
uint8_t auto_run=0; //手动起摆或自动起摆标志位，默认是手动起摆
uint8_t autorun_step0=0; //自动起摆第0步，找到中点，等待起摆
uint8_t autorun_step1=1; //自动起摆第1步
uint8_t autorun_step2=0; //自动起摆第2步
long Target_Position;//目标位置
float D_Angle_Balance; //摆杆角度变化率
long success_count=0;//摆杆在平衡位置的成功次数记录
uint8_t success_flag=0; //自动起摆时满足平衡次数可以起摆的标志位
...
void Auto_run(void)
{
    ...
    static uint8_t help_count=0;
    static uint16_t pid_adjust=0;
...
int Mean_Filter(int sensor)
{
  ...
  static  int16_t Speed_Buf[FILTERING_TIMES]={0};
  ...
  Filter_Speed = (int16_t)(Sum_Speed / FILTERING_TIMES);//
    return Filter_Speed;
}
